	       ...
	       unsigned char flag)
{
	...
	void *buf;
	loff_t infile_size = infile_ptr->f_inode->i_size;
	struct file* temp_ptr = NULL;
	bool failed = false;
	...
	temp_ptr = filp_open((char *)"outfile.temp", O_WRONLY|O_CREAT|O_TRUNC , infile_ptr->f_mode);
	if(IS_ERR(temp_ptr)){
		ret = PTR_ERR(temp_ptr);
		goto out_kfree_buf;
	}
	...
	out_write_in_file:
		bytes_wrote = kernel_write(temp_ptr, buf, bytes_read,
					   &temp_ptr->f_pos);
		if (bytes_wrote < 0) {
...
out_clean_cipher_handles:
	...
	if(!failed){
		ret = vfs_rename(temp_ptr->f_path.dentry->d_parent->d_inode, temp_ptr->f_path.dentry, outfile_ptr->f_path.dentry->d_parent->d_inode, outfile_ptr->f_path.dentry, NULL, 0);			
		if (ret) {
			printk(KERN_ERR "Error in vfs_rename;");
			goto out_read_write;
		}	
	}else{
		ret = vfs_unlink(temp_ptr->f_path.dentry->d_parent->d_inode, temp_ptr->f_path.dentry, NULL);
		if (ret) {
